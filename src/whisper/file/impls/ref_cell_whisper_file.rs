...
pub struct RefCellWhisperFile {
    pub handle: RefCell<File>,
    pub header: Header
}

impl fmt::Debug for RefCellWhisperFile {
...
impl fmt::Display for RefCellWhisperFile {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let ref metadata = self.header.metadata;
        let ref archive_infos = self.header.archive_infos;
...
impl WhisperFile for RefCellWhisperFile {
    fn open(path: &Path) -> Result<RefCellWhisperFile, Error> {
        let file = try!(OpenOptions::new().read(true).write(true)
                            .create(false).open(path));

        let header = try!(read_header(&file));
        let whisper_file = RefCellWhisperFile { header: header, handle: RefCell::new(file) };

        Ok(whisper_file)
    }

    fn new(path: &Path, schema: Schema /* , _: Metadata */) -> Result<RefCellWhisperFile, Error> {
        let opened_file = try!(OpenOptions::new().read(true).write(true).create(true).open(path));
        RefCellWhisperFile::write_data_layout(opened_file, schema)
    }
...
impl RefCellWhisperFile {
    fn write_data_layout(opened_file: File, schema: Schema /* , _: Metadata */) -> Result<RefCellWhisperFile, Error> {
        ...
        let new_whisper_file = RefCellWhisperFile {
    ...
    fn write_through_all_archives(&self,
        ...
        // Write in to the base archive
        {
            ...
            let h_res_base_bucket = h_res.anchor_bucket_ref_mut(file_handle);
    ...
    fn downsample(&self, h_res_archive: &ArchiveInfo,
        ...
        aggregated_value.map(|aggregate| {
            ...
            let l_res_anchor = l_res_archive.anchor_bucket_ref_mut(self.handle.borrow_mut());
...
mod tests {
    ...
    fn build_60_1440_wsp(prefix: &str) -> RefCellWhisperFile {
        ...
        RefCellWhisperFile::new(path.as_path(), schema).unwrap()
    }

    fn build_60_1440_1440_168_10080_52(prefix: &str) -> RefCellWhisperFile {
    ...
    fn bench_opening_a_file(b: &mut Bencher) {
        ...
        b.iter(|| RefCellWhisperFile::open(path).unwrap() );
    }
    ...
    fn test_read_point() {
        let path = Path::new("test/fixtures/60-1440.wsp");
        let file = RefCellWhisperFile::open(path).unwrap();
    ...
    #[test]
    fn test_read_points() {
        let file = RefCellWhisperFile::open(Path::new("test/fixtures/60-1440.wsp")).unwrap();
        let offset = file.header.archive_infos[0].offset;
        // read the first point of the first archive
    ...
    fn test_new_file_has_correct_metadata() {
        ...
        let file = RefCellWhisperFile::new(Path::new("test/fixtures/new_has_correct_metadata.wsp"), schema).unwrap();
    ...
    fn test_split_first_archive() {
        let file = RefCellWhisperFile::open(Path::new("test/fixtures/60-1440-1440-168-10080-52.wsp")).unwrap();
        let current_time = time::get_time().sec as u64;
