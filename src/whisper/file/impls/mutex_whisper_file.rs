...
impl fmt::Debug for MutexWhisperFile {
...
impl fmt::Display for MutexWhisperFile {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let ref metadata = self.header.metadata;
        let ref archive_infos = self.header.archive_infos;
...
impl WhisperFile for MutexWhisperFile {
    fn open(path: &Path) -> Result<MutexWhisperFile, Error> {
        let file = try!(OpenOptions::new().read(true).write(true)
                            .create(false).open(path));

        let header = try!(read_header(&file));
        let whisper_file = MutexWhisperFile { header: header, handle: Mutex::new(file) };

        Ok(whisper_file)
    }

    fn new(path: &Path, schema: Schema /* , _: Metadata */) -> Result<MutexWhisperFile, Error> {
        let opened_file = try!(OpenOptions::new().read(true).write(true).create(true).open(path));
        MutexWhisperFile::write_data_layout(opened_file, schema)
    }
...
impl MutexWhisperFile {
    fn write_data_layout(opened_file: File, schema: Schema /* , _: Metadata */) -> Result<MutexWhisperFile, Error> {
        ...
        let new_whisper_file = MutexWhisperFile {
            handle: Mutex::new(opened_file),
            header: Header {
                metadata: metadata,
    ...
    fn write_through_all_archives(&self,
        ...
        {
            let file_handle = self.handle.lock().unwrap();
            let h_res_base_bucket = h_res.anchor_bucket_guard(file_handle);
    ...
    fn downsample(&self, h_res_archive: &ArchiveInfo,
...

        {
            ...
            {
                let file = self.handle.lock().unwrap();
                h_res_archive.read_points_guard(first_index, first_buf, file);
            }

            match second_read {
                Some((second_index, second_buf)) => {
                    let file = self.handle.lock().unwrap();
                    h_res_archive.read_points_guard(second_index, second_buf, file);
                },
                None => ()
            }
        }
        ...
        aggregated_value.map(|aggregate| {
            ...
            let l_res_anchor = l_res_archive.anchor_bucket_guard(self.handle.lock().unwrap());
...
mod tests {
    ...
    use whisper::file::file::WhisperFile;

    use super::super::super::archive_info::{ ArchiveInfo, ArchiveIndex, BucketName };
    use super::MutexWhisperFile;
    ...
    fn build_60_1440_wsp(prefix: &str) -> MutexWhisperFile {
        ...
        MutexWhisperFile::new(path.as_path(), schema).unwrap()
    }

    fn build_60_1440_1440_168_10080_52(prefix: &str) -> MutexWhisperFile {
    ...
    fn bench_opening_a_file(b: &mut Bencher) {
        ...
        b.iter(|| MutexWhisperFile::open(path).unwrap() );
    }
    ...
    fn test_read_point() {
        let path = Path::new("test/fixtures/60-1440.wsp");
        let file = MutexWhisperFile::open(path).unwrap();
    ...
    #[test]
    fn test_read_points() {
        let file = MutexWhisperFile::open(Path::new("test/fixtures/60-1440.wsp")).unwrap();
        let offset = file.header.archive_infos[0].offset;
        // read the first point of the first archive
    ...
    fn test_new_file_has_correct_metadata() {
        ...
        let file = MutexWhisperFile::new(Path::new("test/fixtures/new_has_correct_metadata.wsp"), schema).unwrap();
    ...
    fn test_split_first_archive() {
        let file = MutexWhisperFile::open(Path::new("test/fixtures/60-1440-1440-168-10080-52.wsp")).unwrap();
        let current_time = time::get_time().sec as u64;
